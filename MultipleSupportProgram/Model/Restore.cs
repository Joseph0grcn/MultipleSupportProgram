        ...
        public void RestoreDB(string databaseName, string restoreFilePath, string getConString) {
            ...
            try
            {
                using (SqlConnection con = new SqlConnection(getConString))
                {
                    ...
                    string command = $"USE [master] ALTER DATABASE [{databaseName}] SET SINGLE_USER WITH ROLLBACK IMMEDIATE RESTORE DATABASE [{databaseName}] FROM  DISK = @BackUpPath WITH  FILE = 1,  NOUNLOAD,  REPLACE,  STATS = 5 ALTER DATABASE [{databaseName}] SET MULTI_USER";
